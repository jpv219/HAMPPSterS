...
if min_res*(2*max_diameter)/128 > 14:
    raise ValueError("Max p_diameter doesn't comply with min. res.")

## rename job with current run
os.system(f'mv {path}/job_base.sh {path}/job_{run_name}.sh')

## Assign values to placeholders
os.system(f'sed -i \"s/RUN_NAME/{run_name}/g\" {path}/job_{run_name}.sh')

box_2 = math.ceil(4*pipe_radius*1000)/1000
box_4 = math.ceil(2*pipe_radius*1000)/1000
box_6 = math.ceil(2*pipe_radius*1000)/1000

os.system(f'sed -i \"s/\'box2\'/{box_2}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'box4\'/{box_4}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'box6\'/{box_6}/\" {path}/job_{run_name}.sh')
...
os.system(f'sed -i \"s/\'x_subd\'/{xsub}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'y_subd\'/{ysub}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'z_subd\'/{zsub}/\" {path}/job_{run_name}.sh')

os.system(f'sed -i \"s/\'n_cpus\'/{ncpus}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'n_nodes\'/{n_nodes}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'mem\'/{mem}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'cell1\'/{cell1}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'cell2\'/{cell2}/\" {path}/job_{run_name}.sh')
os.system(f'sed -i \"s/\'cell3\'/{cell3}/\" {path}/job_{run_name}.sh')
