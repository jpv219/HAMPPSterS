...
from LHS_Dataspace import SMX_Sampler
...
max_diameter = 0.028
SMX_dict = {'Bar_Width (mm)': [1,18],'Bar_Thickness (mm)': [1,6],'Radius (mm)': [5,max_diameter*1000/2],'Nbars':[3,16],'Flowrate (m3/s)': [1e-6,1e-4],'Angle':[20,80]}

captured_output = io.StringIO()

LHS_sampler = SMX_Sampler(SMX_dict, nruns)
...
with open('../DOE/LHS_geom_5.pkl', 'wb') as file:
...
if not re_run:
    ...
    with open('../params/parameters_geom_5.csv', 'w', newline='') as csvfile:
    ...
    with open('../params/parameters_geom_5.csv', 'r') as csvfile:
