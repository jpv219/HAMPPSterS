...
case = "osc_clean"
nruns = 50
nruns_list = [str(i + 200) for i in range(1, nruns + 1)]
...
convert_path = ps.plist("convert_path",["/rds/general/user/lchagot/home/F_CONVERT"])
...
osc_dict = {'epsilon': [2,6],'Wave_number (1/m)': [2,3],'Surf_tension (N/m)': [1,10],'Density_l (kg/m3)': [1e-2,2],
            'Density_g (kg/m3)': [1e-3,1e-3],'Viscosity_l (Pa*s)':[1e-3,2e-2], 'Viscosity_g (Pa*s)':[6.7e-6,1e-6],'Gravity (m/s2)': [0,50]}
...
with open('../DOE/LHS_osc_clean_5.pkl', 'wb') as file:
...
with open('../params/parameters_osc_clean_5.csv', 'w', newline='') as csvfile:
