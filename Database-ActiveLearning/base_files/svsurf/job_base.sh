...
#PBS -o RUN_NAME.out
#PBS -j oe
#PBS -lselect=1:ncpus=96:mem=100gb
...
! Size of the domain
  ...
  box= 0.0d0  0.086d0  0.0d0  0.086d0  0.0d0  0.1275d0
!--------------------------------------------------------------------------------------------------------------------------------  
/
...
! Fixed time step,                     If fixed, set dt.
  fixed_time_step=.TRUE.             dt=3.125d-5
!
...
!--------------------------------------------------------------------------------------------------------------------------------
  ...
  ! SOLVER PARAMETERS
  ! Iterations.     Tolerance.          Relaxation.        components (used by GMRES only).
  p_max_iter=100    p_tol=1.0D-12         p_relax=1.05D0     p_max_comp=30
  !
...
!--------------------------------------------------------------------------------------------------------------------------------
  ...
  surfactant_iso_bctype                    = "N"           ! Immersed solid boundary condition type
  ! Faces: West          East
...
! If ParaView,               Format,                Loop Frequency,          Time interval (s),             Prefix.
  paraview_output=.TRUE.    paraview_format="vtk"   paraview_frequency=0    paraview_time_interval=0.00625d0   paraview_file_prefix="RUN_NAME"
!
...
! Restart Output Frequency,   Output time interval (s)      Output File Prefix.
  output_restart_frequency=0     output_restart_time_interval=0.00625d0     output_restart_file_prefix="RUN_NAME"
!
! Restart (true/false),        Input File Index,        Input File Prefix.
  restart=.TRUE.              input_file_index=422       input_file_prefix="RUN_NAME"
