...
!!!!   IMPELLER PARAMETERS
  real(kind=d), parameter :: BLADE_RADIUS        =   0.5_d*'impeller_d'_d    !  (m)
  real(kind=d), parameter :: BLADE_LENGTH        =   0.5_d*'impeller_d'_d    !  (m)
  real(kind=d), parameter :: BLADE_HEIGHT        =   'blade_width'_d     !  (m)
  real(kind=d), parameter :: BLADE_WIDTH         =   'blade_thick'_d    !  (m)
  integer     , parameter :: BLADE_NUMBER        =   'nblades'
  real(kind=d), parameter :: BLADE_ANGLE         =   'inclination'_d     ! (Degree)
  real(kind=d), parameter :: Clearance           =   'clearance'_d     !  (m)
  ...
  real(kind=d), parameter :: Impeller_Frequency  =   'frequency'_d
 

  public :: USER_solid, USER_bcd 
...
!***********************************************************************
    ...
    real(kind=d)    :: b0, b1, b, blades, BLADE_CIRCULAR_RADIUS 
    real(kind=d)    :: c1, c2, p1, p2, p3, DISK, tank, TUBE_DISK

    BLADE_CIRCULAR_RADIUS = BLADE_RADIUS - 0.5d0*BLADE_LENGTH
    x0 = 0.5_d*(BOX(1)+BOX(2))
    y0 = 0.5_d*(BOX(3)+BOX(4))
    z0 = BOX(5) + Clearance
...
!!! BLUID BLADES 
        Theta     = 0.0_d
        XX        = x0 + BLADE_CIRCULAR_RADIUS * cos(Theta)
        YY        = y0 + BLADE_CIRCULAR_RADIUS * sin(Theta)
        ZZ        = z0
      ...
      b0           =   blade0%distance(obj%subobj(1)%form)
      ...
      do i = 1 , BLADE_NUMBER-1
        ...
        b1= blade%distance(obj%subobj(1)%form)
        b = union(b1,b0)
        b0= b
      end do      
...
!!! BUILD DISK       
      ...
      p1 = pl1%distance(obj%subobj(1)%form)
      p2 = pl2%distance(obj%subobj(1)%form)
      c2 = cyl2%distance(obj%subobj(1)%form)
    ...
    obj%subobj(1)%distance = union(DISK, blades)
    obj%subobj(1)%movement = movement_property(FORCED=.TRUE., STATIC=.FALSE., &
					       TRANSLATION_VELOCITY=[0.0_d, 0.0_d, 0.0_d], & 
					       ROTATION_VELOCITY=[0.0_d, 0.0_d, 2.0_d*BLUE_PI_VALUE*Impeller_Frequency])
...
program waves
 ...
 !! Set VAR.
 ! call VAR_set(COM=com, MSH=msh, VAR=var, USER_INTERFACE=USER_int, USER_SOLID=USER_solid, USER_BC=USER_bcd)

  ! Set VAR.
  call VAR_set(COM=com, MSH=msh, VAR=var, USER_SOLID=USER_solid, USER_BC=USER_bcd)
